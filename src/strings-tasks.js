...
function removeFirstOccurrences(str, value) {
  ...
  const extractedValue = str.slice(valueIndex, valueIndex + valueLength);
  return str.replace(extractedValue, '');
}
...
function removeLastOccurrences(str, value) {
  ...
  if (str.includes(value)) {
    ...
    const replaced =
      str.slice(0, valueIndex) + str.slice(valueIndex + valueLength);
...
function sumOfCodes(str) {
  ...
  return codes.reduce((acc, curr) => acc + curr, 0);
}
...
function isPalindrome(str) {
  const noSpaceString = str.toLowerCase().split(' ').join('');
  let noComaString = noSpaceString.split(',').join('');
  if (
    noComaString[noComaString.length - 1] === '!' ||
    noComaString[noComaString.length - 1] === '?'
  ) {
    noComaString = noComaString.slice(0, -1);
  }
  return noComaString.split('').reverse().join('') === noComaString;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arrSentence.length; i += 1) {
    if (arrSentence[i].length > longestWord.length) {
      longestWord = arrSentence[i];
    }
  }
  return longestWord;
}
...
function encodeToRot13(str) {
  ...
  const encodedArr = strArr.map((word) => {
    let ecodedWord = '';
    for (let i = 0; i < word.length; i += 1) {
      const index = input.indexOf(word[i]);
      ecodedWord += output[index];
    }
